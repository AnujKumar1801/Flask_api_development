...
ball_withmatch['BowlingTeam'] = ball_withmatch[[
  'BowlingTeam', 'BattingTeam'
]].apply(lambda x: x.values[0].replace(x.values[1], ''), axis=1)
batter_data = ball_withmatch[np.append(balls.columns.values,
                                       ['BowlingTeam', 'Player_of_Match'])]


def team_record(team_name):
  home_matches = len(matches[matches['Team1'] == team_name])
  home_matches_win = len(matches[(matches['Team1'] == team_name)
                                 & (matches['WinningTeam'] == team_name)])
  away_matches = len(matches[matches['Team2'] == team_name])
  away_matches_win = len(matches[(matches['Team2'] == team_name)
                                 & (matches['WinningTeam'] == team_name)])
  ...
  result_dictionary = {
    ...
    'home_win': home_matches_win,
    'away_win': away_matches_win,
    'total_win': home_matches_win + away_matches_win,
    'win%':
    f"{ round(((home_matches_win + away_matches_win) / total_matches) * 100,2)}%",
    'homewin%': f"{ round(((home_matches_win) / home_matches) * 100,2)}%",
    'awaywin%': f"{ round(((away_matches_win) / away_matches) * 100,2)}%"
  }
...
bowler_data['bowler_run'] = bowler_data[['extra_type',
                                         'total_run']].apply(bowlerRun, axis=1)
...
bowler_data['isBowlerWicket'] = bowler_data[['kind', 'isWicketDelivery'
                                             ]].apply(bowlerWicket, axis=1)
...
def bowlerRecord(bowler, df):
  ...
  best_wicket = gb.sort_values(['isBowlerWicket', 'bowler_run'],
                               ascending=[False, True
                                          ])[['isBowlerWicket',
                                              'bowler_run']].head(1).values
  if best_wicket.size > 0:

    best_figure = f'{best_wicket[0][0]}/{best_wicket[0][1]}'
  else:
    best_figure = np.nan
  mom = df[df.Player_of_Match == bowler].drop_duplicates('ID',
                                                         keep='first').shape[0]
  data = {
    'innings': inngs,
    'wicket': wicket,
